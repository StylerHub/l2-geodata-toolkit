...
def level_extra_triangles(pkg):
    ...
    # попадают сюда, но их отсеивает вызывающая сторона по меш-футпринту
    # (полы интерьеров лежат под меш-стенами/крышами, а вода/зоны — на
    # открытой воде без мешей) — см. generate_region_full.

...
WALK_NZ = 0.766        # |nz|/|n| ≥ cos(40°) → пол (наклон ≤40°). Знак нормали НЕ
                       # важен: коллизия UE2 двусторонняя, winding в мешах L2
                       # случайный (~66% пологих граней «смотрят вниз», у toh_Stair02
                       # так вся лестница) — угол меряется без знака
...
# Окно стены «на уровне груди» (хит 10..32 над полом =
# стена; ниже — переступаешь, выше — проходишь под). Любое попадание закрывает
# направление (OR), в отличие от прежнего AND(низ, верх), дававшего и ложные
# стены (бордюр+карниз с разных граней), и дыры (парапет 30..45 без низа/верха).
WALL_RAY_OFFS = (12.0, 20.0, 28.0)   # высоты лучей над полом внутри окна (10,32)
WALL_LAT_OFFS = (-7.0, 0.0, 7.0)     # 3 параллельные линии поперёк движения
                                     # — ловят смещённые
                                     # от центра колонны/стены
BLOCK_WIN_LO = 10      # BlockingVolume заграждает слой, только если z-диапазон
BLOCK_WIN_HI = 32      # его крутых граней пересекает окно (пол+10 … пол+32)
STAIR_MAX_RISE = 16    # крутая грань с размахом по Z ≤ шага (= UP_STEP) — под-
                       # ступенок/бордюр, переступается и стеной не считается
...
def _prep_geometry(tris, west, north):
    ...
    for (ax, ay, az), (bx, by, bz), (cx, cy, cz) in tris:
        ...
        if abs(nz) < WALK_NZ * math.sqrt(ln2):        # крутая → стена
            if max(az, bz, cz) - min(az, bz, cz) <= STAIR_MAX_RISE:
                continue                               # подступенок/бордюр ≤ шага
            wi = len(walls)
            walls.append((ax, ay, az, ux, uy, uz, vx, vy, vz))  # e1=(u), e2=(v)
            grid = w_grid
            idx = wi
        else:                                          # пологая → пол (двусторонняя)
            ...
            # флаг is_floor всегда True: по знаку нормали пол от потолка не
            # отличить (winding случайный). Роль потолков в клиренсе играют
            # такие же полы-кандидаты выше; изнанка тонкой плиты сливается
            # LAYER_MERGE, толстой — гасится клиренсом от собственного верха,
            # своды — отсекаются _prune_unreachable
...
def _wall_between(px, py, npx, npy, z, w_idx, walls):
    """X/Y-луч: проход закрыт, если крутая грань пересекает горизонтальный
    отрезок [центр→сосед] в окне корпуса (окно 10..32 над полом): лучи
    на высотах WALL_RAY_OFFS, 3 параллельные линии со смещениями WALL_LAT_OFFS
    поперёк движения. ЛЮБОЕ попадание = стена: бордюр ниже окна переступаем,
    свод/карниз выше — проходим под. Подступенки лестниц отфильтрованы ещё в
    _prep_geometry (z-размах ≤ STAIR_MAX_RISE). Рёбра e1,e2 предпосчитаны;
    Möller–Trumbore для сегмента."""
    dx = npx - px
    dy = npy - py
    lx, ly = (1.0, 0.0) if dx == 0 else (0.0, 1.0)   # поперёк движения
    for wi in w_idx:
...
def _span_layers(floor_zs, ceil_zs, base):
    ...
    Рельеф (base) не гасится ПОТОЛКАМИ (дно валуна/кроны над землёй не выжигает
    дыр в поле — земля под навесом/аркой проходима), но гасится принятым ПОЛОМ
    ближе AGENT_HEIGHT над ним: между ними всё равно не встать, а слой-призрак
    земли под полом/лестницей ловит стены кромок — полоса глухих клеток у
    подножия (в классической геодате рельеф отдельным слоём не существует вовсе). Близкие
    полы (<LAYER_MERGE) сливаются — артефакт округления высот."""
    terrain = base[0] if base else None
    cand = sorted(set(list(floor_zs) + list(base)), reverse=True)
    ceils = sorted(set(ceil_zs))
    out = []
    for z in cand:
        # окно слива: рельеф гасит любой принятый пол в пределах клиренса,
        # мешевые полы сливаются только в упор (округление высот)
        if out and out[-1] - z < (AGENT_HEIGHT if z == terrain else LAYER_MERGE):
            continue
        if z != terrain:                          # потолки/днища мешей рельеф не гасят
...
def _blocking_edges(blocking, west, north):
    """BlockingVolume-грани → закрытые границы ячеек (заградительные стены
    уровня: край играбельной зоны, перекрытые дороги). Возвращает dict
    ключ → (zlo, zhi): ('x', gx, gy) — граница между (gx-1,gy) и (gx,gy);
    ('y', gx, gy) — между (gx,gy-1) и (gx,gy); (zlo, zhi) — z-диапазон граней
    объёма на этой границе (заграждает только слои, чьё окно корпуса
    BLOCK_WIN_LO..HI он пересекает — объём на мосту не запирает пол под ним).
    Берутся только КРУТЫЕ грани: горизонтальные крышки объёма — не стены, их
    диагонали рисовали бы полосы глухих ячеек поперёк пола. Крупные грани
    (>BIG_PLANE) — заградительный объём ВОКРУГ зоны/локации, а не внутренняя
    стена: их растеризация заперла бы всю зону изнутри, поэтому пропускаем."""
    blocked = {}
    ...
    for tri in blocking:
        ...
        if ln2 <= 0 or abs(nz) >= WALK_NZ * math.sqrt(ln2):
            continue                               # пологая крышка/дно — не стена
            ...
            for s in range(steps + 1):
                ...
                if prev is not None and (gx, gy) != prev:
                    ...
                    if gx != pgx and gy != pgy:        # диагональ — метим обе границы
                        _add(('x', max(gx, pgx), gy), zlo, zhi)
                        _add(('y', gx, max(gy, pgy)), zlo, zhi)
                ...
                for cy in range(8):
                    ...
                    for z in ls:
                        ...
                        for di, (bit, ngx, ngy, bkey) in enumerate((
                            ...
                            rng = blocked.get(bkey)      # BlockingVolume: заграждает,
                            if rng is not None and (     # только если его z-диапазон
                                    rng[0] < z + BLOCK_WIN_HI    # пересекает окно
                                    and rng[1] > z + BLOCK_WIN_LO):  # корпуса слоя
                                nswe &= ~bit
                                if height_ok:
                                    block_bits |= bit
                                continue
